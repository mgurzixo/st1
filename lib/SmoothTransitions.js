...
function makeShadow(app, id) {
    ...
    console.log(`[makeShadow] left=${gbr.left}`);
...
export async function stGoto(action, theOptions) {
    ...
    let bcr = app.getBoundingClientRect();
    console.log(bcr);
    console.log(`[stGoto] bct.top=${bcr.top} top=${csApp.top}`);
    // Don't know why, but there is 4 px difference
    gbrExtraX = bcr.left;
    gbrExtraY = bcr.top;
